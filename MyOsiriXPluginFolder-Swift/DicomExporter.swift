...
// MARK: - DICOM Export Module

/// Handles exporting DICOM series from Horos to the filesystem.
final class DicomExporter {
    ...
    enum StudyExportError: LocalizedError {
        case noSeries
    ...
    private static let supportedModalities: Set<String> = ["CT", "MR"]

    // MARK: - Singleton

    static let shared = DicomExporter()

    private init() {}

    // MARK: - Public Methods

    /// Export the active series from a viewer controller.
    ///
    /// - Parameter viewer: The viewer controller containing the active series
    /// - Returns: Export result with exported files information
    /// - Throws: ActiveSeriesExportError if export fails
    func exportActiveSeries(from viewer: ViewerController) throws -> ExportResult {
        ...
        guard let modality = normalizedModality, Self.supportedModalities.contains(modality) else {
    ...
    /// Export all compatible series from a study.
    ///
    /// - Parameter study: The DICOM study to export
    /// - Returns: Export result with all exported series
    /// - Throws: StudyExportError if no compatible series found or export fails
    func exportCompatibleSeries(from study: DicomStudy) throws -> ExportResult {
        ...
        guard !seriesArray.isEmpty else {
            throw StudyExportError.noSeries
        }
        ...
        guard !compatibleSeries.isEmpty else {
            throw StudyExportError.noCompatibleSeries
        }
        ...
        do {
            ...
            throw StudyExportError.exportFailed(underlying: error)
        }
    ...
    /// Normalize a series collection to an array.
    ...
    /// Normalize paths from various collection types.
